...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function toFavorite() {
    const favorites = document.querySelectorAll(".header__favorite, .apartments-aside__favorite");
  ...
  function cardSlider() {
    document.querySelectorAll(".card__slider").forEach((sliderEl) => {
      ...
      const realSlides = sliderEl.querySelectorAll(".swiper-wrapper > .swiper-slide:not(.swiper-slide-duplicate)");
      ...
      for (let i = 0; i < slidesCount; i++) {
        ...
        b.dataset.index = i;
        bottomPagination.appendChild(b);
      }

      const bottomBullets = bottomPagination.querySelectorAll(".swiper-pagination-bullet");

      bottomBullets.forEach((b) => {
        b.addEventListener("click", () => {
          swiper.slideToLoop(Number(b.dataset.index));
        });
      });

      const updateBottom = () => {
        bottomBullets.forEach((b) => b.classList.remove("swiper-pagination-bullet-active"));
        const real = swiper.realIndex;
        if (bottomBullets[real]) bottomBullets[real].classList.add("swiper-pagination-bullet-active");
      };

      swiper.on("slideChange", updateBottom);
      updateBottom();

      if (window.innerWidth > 768) {
        const hoverBullets = topPagination.querySelectorAll(".swiper-pagination-bullet");
        hoverBullets.forEach((bullet, index) => {
  ...
  function initPricesAccordion() {
    document.querySelectorAll(".prices-table__block").forEach((block) => {
      const row = block.querySelector(".prices-table__row");
      const content = block.querySelector(".prices-table__content");
